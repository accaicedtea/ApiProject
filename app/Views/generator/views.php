<section class="container-fluid py-4">
                    ...
                    <div class="card h-100 shadow-sm <?= $isEnabled ? 'border-success' : '' ?>">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex gap-2">
                                ...
                                <form method="POST" action="<?= url('/generator/delete-view') ?>" class="flex-fill"
                                    id="deleteForm_<?= e($name) ?>">
                                    <input type="hidden" name="csrf_token" value="<?= e($csrf_token) ?>">
                                    <input type="hidden" name="view_name" value="<?= e($name) ?>">
                                    <button type="button" class="btn btn-danger btn-sm w-100"
                                        onclick="AppModal.confirm('Sei sicuro di voler eliminare la vista <strong><?= e($name) ?></strong>?', () => document.getElementById('deleteForm_<?= e($name) ?>').submit(), 'Elimina Vista')">
    ...
    <div class="text-center mt-5">
        <a href="<?= url('/generator') ?>" class="btn btn-outline-secondary">
...
async function toggleView(viewName) {
    ...
    try {
        ...
        const response = await fetch('<?= url("/generator/toggle-view") ?>', {
            method: 'POST',
            body: formData
        });
        ...
        if (result.success) {
            ...
            AppModal.error(result.error, 'Errore Toggle Vista');
        }
    } catch (error) {
        checkbox.checked = originalState;
        AppModal.error('Errore durante l\'aggiornamento: ' + error.message, 'Errore di Rete');
    }
}
...
async function testQuery() {
    ...
    if (!query.trim()) {
        AppModal.warning('Inserisci una query SQL prima di testare.', 'Query Mancante');
        return;
    }
    ...
    try {
        const response = await fetch('<?= url("/generator/test-view") ?>', {
            method: 'POST',
            body: formData
        });
        ...
        AppModal.error('Errore durante il test della query: ' + error.message, 'Errore Test');
        saveBtn.disabled = true;
    }
}
...
function saveView() {
    ...
    if (!name || !query) {
        AppModal.warning('Nome e query sono obbligatori.', 'Campi Mancanti');
        return;
    }
    ...
    form.method = 'POST';
    form.action = '<?= url("/generator/save-view") ?>';
...
function editView(viewName) {
    ...
    if (!view) {
        AppModal.error('Vista non trovata.', 'Errore');
        return;
    }
